...
class GenerateUnitTest(GenerateTestClass):
    ...
    def update_config_file(self):
        ...
        if not os.path.exists(_UNIT_TESTS_CONFIG):
            raise FileNotFoundError(f"ERROR: {_UNIT_TESTS_CONFIG} not found.")

        with open(_UNIT_TESTS_CONFIG, "a", encoding="utf-8") as f:
...
class GenerateFunctionalTest(GenerateTestClass):
    ...
    def update_config_file(self):
        ...
        if not os.path.exists(_FUNCTIONAL_TESTS_CONFIG):
            raise FileNotFoundError(f"ERROR: {_FUNCTIONAL_TESTS_CONFIG} not found.")

        with open(_FUNCTIONAL_TESTS_CONFIG, "a", encoding="utf-8") as f:
            f.write(f"\n[{self.test_name}]\n")
            f.write(f"test_dir = {self.build_dir}\n")
            f.write(f"test_exe = {self.executable_name}\n")
            f.write("out_file = None\n")
            f.write("use_param_file = False\n")
            f.write("other_args = []\n")
