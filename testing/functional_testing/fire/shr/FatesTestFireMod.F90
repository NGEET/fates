...
contains
   ...
   subroutine WriteFireData(out_file, nsteps, nfuelmods, temp_degC, precip, rh, NI,     &
      loading, frac_loading, fuel_BD, fuel_SAV, non_trunk_loading, weighted_loading_dead,     &
      weighted_loading_live, fuel_moisture_dead, fuel_moisture_live, fuel_MEF_dead, fuel_MEF_live, &
      ...
      real(r8),           intent(in) :: weighted_loading_dead(:)
      real(r8),           intent(in) :: weighted_loading_live(:)
      real(r8),           intent(in) :: fuel_moisture_dead(:,:)
      real(r8),           intent(in) :: fuel_moisture_live(:,:)
      real(r8),           intent(in) :: fuel_MEF_dead(:,:)
      real(r8),           intent(in) :: fuel_MEF_live(:,:)
      ...
      integer              :: wgt_loadingdID, wgt_loadinglID
      integer              :: BDID, SAVID
      integer              :: moistdID, moistlID
      integer              :: cID, mefdID, meflID
      ...
      ! register fuel moisture of dead fuel
      call RegisterVar(ncid, 'fuel_moisture_dead', (/dimIDs(1), dimIDs(3)/), type_double,   &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],                   &
         [character(len=150) :: 'time fuel_model', 'm3 m-3', 'average fuel moisture of dead fuels'],  &
         3, moistdID)

      ! register fuel moisture of live fuel
      call RegisterVar(ncid, 'fuel_moisture_live', (/dimIDs(1), dimIDs(3)/), type_double,   &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],                   &
         [character(len=150) :: 'time fuel_model', 'm3 m-3', 'average fuel moisture of live fuels'],  &
         3, moistlID)

      ! register dead fuel MEF
      call RegisterVar(ncid, 'fuel_MEF_dead', (/dimIDs(1), dimIDs(3)/), type_double,   &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],                   &
         [character(len=150) :: 'time fuel_model', 'm3 m-3', 'average dead fuel moisture of extinction'],  &
         3, mefdID)

      ! register dead fuel MEF
      call RegisterVar(ncid, 'fuel_MEF_live', (/dimIDs(1), dimIDs(3)/), type_double,   &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],                   &
         [character(len=150) :: 'time fuel_model', 'm3 m-3', 'average live fuel moisture of extinction'],  &
         3, meflID)
      ...
      ! register weighted dead fuel loading
      call RegisterVar(ncid, 'weighted_loading_dead', dimIDs(3:3), type_double,    &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],       &
         [character(len=150) :: 'fuel_model', 'kgC m-2', 'weighted total dead loading'],  &
         3, wgt_loadingdID)

      ! register weighted live fuel loading
      call RegisterVar(ncid, 'weighted_loading_live', dimIDs(3:3), type_double,    &
         [character(len=20)  :: 'coordinates', 'units', 'long_name'],       &
         [character(len=150) :: 'fuel_model', 'kgC m-2', 'weighted total live loading'],  &
         3, wgt_loadinglID)
      ...
      call WriteVar(ncid, wgt_loadingdID, weighted_loading_dead(:))
      call WriteVar(ncid, wgt_loadinglID, weighted_loading_live(:))
      call WriteVar(ncid, moistdID, fuel_moisture_dead(:,:))
      call WriteVar(ncid, moistlID, fuel_moisture_live(:,:))
      call WriteVar(ncid, BDID, fuel_BD(:))
      call WriteVar(ncid, SAVID, fuel_SAV(:))
      call WriteVar(ncid, mefdID, fuel_MEF_dead(:,:))
      call WriteVar(ncid, meflID, fuel_MEF_live(:,:))
