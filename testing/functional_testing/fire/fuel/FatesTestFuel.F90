program FatesTestFuel
   ...
   real(r8),                          allocatable :: weighted_loading_dead(:)  ! total dead fuel loading weighted [kgC/m2]
   real(r8),                          allocatable :: weighted_loading_live(:)  ! total live fuel loading weighted [kgC/m2]
   ...
   real(r8),                          allocatable :: fuel_moisture_dead(:,:)   ! dead fuel moisture [m3/m3]
   real(r8),                          allocatable :: fuel_moisture_live(:,:)   ! live fuel moisture [m3/m3]
   real(r8),                          allocatable :: fuel_MEF_dead(:,:)        ! dead fuel moisture of extinction [m3/m3
   real(r8),                          allocatable :: fuel_MEF_live(:,:)        ! live fuel moisture of extinction [m3/m3
   ...
   allocate(NI(n_days))
   allocate(fuel_moisture_dead(n_days, num_fuel_models))
   allocate(fuel_moisture_live(n_days, num_fuel_models))
   allocate(fuel_MEF_dead(n_days, num_fuel_models))
   allocate(fuel_MEF_live(n_days, num_fuel_models))
   ...
   allocate(weighted_loading_dead(num_fuel_models))
   allocate(weighted_loading_live(num_fuel_models))
   ...
   do f = 1, num_fuel_models
      ...
      call fuel%CalculateWeightingFactor(SF_val_SAV, SF_val_part_dens)
      ...
      weighted_loading_dead(f) = fuel(f)%weighted_loading_dead
      weighted_loading_live(f) = fuel(f)%weighted_loading_live
      frac_loading(:,f) = fuel(f)%frac_loading(:)
      fuel_BD(f) = fuel(f)%bulk_density_weighted
      fuel_SAV(f) = fuel(f)%SAV_weighted
   ...
   do i = 1, n_days
      ...
      do f = 1, num_fuel_models
         ...
         fuel_moisture_dead(i, f) = fuel(f)%average_moisture_dead
         fuel_moisture_live(i, f) = fuel(f)%average_moisture_live
         fuel_MEF_dead(i, f) = fuel(f)%MEF_dead
         fuel_MEF_live(i, f) = fuel(f)%MEF_live
      end do
   end do

   ! write out data
   call WriteFireData(out_file, n_days, num_fuel_models, temp_degC, precip, rh, NI,       &
      fuel_loading, frac_loading, fuel_BD, fuel_SAV, non_trunk_loading, weighted_loading_dead,  &
      weighted_loading_live, fuel_moisture_dead, fuel_moisture_live, fuel_MEF_dead,       &
      fuel_MEF_live, fuel_models, carriers)
