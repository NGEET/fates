...
contains
  ...
  subroutine FatesPlantRespPhotosynthDrive (nsites, sites,bc_in,bc_out,dtime)
    ...
    real(r8) :: flush_frac            ! Fraction of the number of active leaf layers
                                      ! over the maximum derived from allometry
    ...
    ! When we remember the carbon balance of different leaf layers, if
    ! the leaves are not fully flushed, we have options. If this is .false.
    ! we assume that the leaf layer that we get cbal from matches the memory
    ! layer by simply integrating leaf widths. Or, if this is .true., we
    ! stretch out the leaf layers that we get cbal from so they are distributed
    ! verticaly over the same bin depth as the allometric maximum, albeit a
    ! little dilluted.
    
    logical, parameter :: stretch_cbalmem_vert = .false.
                ...
                if(currentPatch%countcohorts > 0.0)then   ! Ignore empty patches
                   ...
                   do while (associated(currentCohort)) ! Cohort loop
                      ...
                      canopy_mask_if: if(currentPatch%canopy_mask(cl,ft) == 1)then
                         ...
                         leaf_layer_loop: do iv = 1,currentCohort%nveg_act
                         ...
                         call ScaleLeafLayerFluxToCohort(nva,         & !in
                         ...
                         ! Only track the last few leaf layer bins
                         ...
                         ! being in a deciduous transition. So when the plant
                         ! is on-allometry, then nveg_act==nveg_max and we 
                         ! record the carbon balance in all memory bins. If it is
                         ! not fully flushed, then we may not even be able to record
                         ! 
                         ...
                         if(nva>0) then
                            ...
                            if(.not.stretch_cbalmem_vert)then
                               ...
                               do ivm = 1,min(nlevleafmem,nva)
                                  ...
                                  flush_frac = real(nva,r8)/real(nvm,r8)
...
subroutine ScaleLeafLayerFluxToCohort(nveg_act,          & ! in   currentCohort%nveg_act
   psn_llz,     & ! in   %psn_z(1:currentCohort%nveg_act,ft,cl)
   lmr_llz,     & ! in   lmr_z(1:currentCohort%nveg_act,ft,cl)
   rs_llz,      & ! in   rs_z(1:currentCohort%nveg_act,ft,cl)
   elai_llz,    & ! in   %elai_profile(cl,ft,1:currentCohort%nveg_act)
   c13disc_llz, & ! in   c13disc_z(cl, ft, 1:currentCohort%nveg_act)
