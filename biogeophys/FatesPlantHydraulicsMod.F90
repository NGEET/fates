...
contains 
  ...
  subroutine KmaxInnerShell(currentSite, ccohort, hksat_soil, kmax_innershell)

    ! -----------------------------------------------------------------------------------
    ! This subroutine calculates update the conductance across the soil to root
    ! interface depending on the plant water uptake/loss. 
    ...
    !
    ! Output: conductance at soil-root inerface (each layer):
    ! ccohort_hydr%kmax_innershell(j)                  [kg s-1 MPa-1 ]

    ! Consider M Williams et al. 1996
    ! Consider Kennedy et al. 2019
    ! https://agupubs.onlinelibrary.wiley.com/doi/full/10.1029/2018MS001500
    ! 
    ! flux = conductance * area * (delta psi + delta z)
    !
    !
    ...
    real(r8),intent(in)                 :: hksat_soil(:)      ! Saturated Hydraulic conductivity 
                                                              ! of the soil [mm H2O s-1]
    real(r8),intent(out)                :: kmax_innershell(:)

    type(ed_cohort_hydr_type),pointer :: ccohort_hydr  ! Plant hydraulics structure
    type(ed_site_hydr_type),pointer :: csite_hydr

    integer  :: j                                      ! loop index for soil layers
    real(r8) :: hksat_s                                ! hydraulic conductivity at saturation [kg s-1 MPa-1]
    ...
    !          * 1e6 [Pa MPa-1]
    !                           = [kg s-1 m-1 MPa-1]
    real(r8) :: kmax_root_surf_total                   ! maximum conducitivity for total 
                                                       ! root surface [kg MPa-1 s-1]
    real(r8) :: kmax_soil_total                        ! maximum conducitivity for from root
                                                       ! surface to soil shell(kg water/Mpa/s) 
                                                       ! which is equiv to   [kg m-1 s-1 MPa-1]    
    real(r8) :: kmax_root_surf                         ! maximum conducitivity for unit root 
                                                       ! surface (kg water/m2 root area/Mpa/s) 

    integer, parameter :: k_inner = 1                  ! index for the inner rhizosphere shell (closest to root)
    logical, parameter :: use_kmax_rootsurf_smoothing = .false.  ! If this is true, use a root surface conductivity
    ! that is a linear function of the potential ratio of
    ! the absorbing root and the inner shell
    ...
    do j=1, csite_hydr%nlevsoi_hyd
       ...
       hksat_s = hksat_soil(j) * m_per_mm * 1._r8/grav_earth * pa_per_mpa

       psi_ratio = ccohort_hydr%psi_aroot(j)/csite_hydr%psisoi_liq_innershell(j)

       if(use_kmax_rootsurf_smoothing) then

          psi_ratio = ccohort_hydr%psi_aroot(j)/csite_hydr%psisoi_liq_innershell(j)

          ! if psi_ratio > 1.5, then the root is pretty dry compared to the soil
          ! and we use the hydr_kmax_rsurf1.  If it below 0.667, then the soil
          ! is pretty dry compared to root, and use hydr_kmax_rsurf2

          slope = (hydr_kmax_rsurf1-hydr_kmax_rsurf2)/(1.5-0.666)

          kmax_root_surf = hydr_kmax_rsurf2 + min(hydr_kmax_rsurf1,max(0._r8,(psi_ratio-0.666)*slope))
       ...
       ! kmax_root_surf, conductivity on root surface, units: [kg m-2 Mpa-1 s-1]
       ! Surface area uses a cylinder model: A = L*2*Pi*R
       !                                     A = l_aroot_layer * 2 * Pi * rs1

       kmax_root_surf_total = kmax_root_surf*2._r8*pi_const *csite_hydr%rs1(j)* &
            csite_hydr%l_aroot_layer(j)	 

       ! If the nodal radius of the rhizosphere is smaller than the root radius
  ...
  subroutine updateSizeDepTreeHydStates(currentSite,ccohort)
    ...
    do j=1,currentSite%si_hydr%nlevsoi_hyd
        ...
        ccohort_hydr%errh2o_growturn_aroot = ccohort_hydr%errh2o_growturn_aroot + & 
              denh2o*cCohort%n/AREA*(ccohort_hydr%th_aroot(j)-th_aroot_uncorr(j))*ccohort_hydr%v_aroot_layer(j)
    enddo

    ! Storing mass balance error
    ! + means water created; - means water destroyed
    ccohort_hydr%errh2o_growturn_ag(:) = denh2o*cCohort%n/AREA*ccohort_hydr%v_ag(:) * & 
                                         (ccohort_hydr%th_ag(:)-th_ag_uncorr(:))
    ccohort_hydr%errh2o_growturn_troot = denh2o*cCohort%n/AREA*ccohort_hydr%v_troot * &
  ...
  subroutine UpdateSizeDepRhizVolLenCon(currentSite, bc_in)
    ...
    !update the conductitivity for first soil shell is done at subroutine KmaxInnerShell
  ...
  subroutine hydraulics_bc ( nsites, sites,bc_in,bc_out,dtime )
    ...
    total_e = site_hydr%h2oveg-(prev_h2oveg + totalrootuptake )
  ...
  subroutine Hydraulics_1DSolve(ccohort, ft, z_node, v_node, ths_node, thr_node, kmax_bound, &
    ...
    real(r8) :: hdiff_bound(  n_hypool_tot)      ! total water potential difference across lower boundary          [MPa]
    ...
    ! Constants governing solution convergence
    integer,parameter   :: maxiter         = 5         ! maximum iterations for timestep reduction                       [-]
    integer,parameter   :: imult           = 3         ! iteration index multiplier                                      [-]
    ...
    do while( iterh1 == 0 .or. ((abs(we_local) > thresh .or. supsub_flag /= 0) .and. iterh1 < maxiter) )
       ...
       do while( iterh2 < dt_fac .and. ((abs(we_local) <= thresh .and. &
          ...
          call boundary_hdiff_and_k( n_hypool_tot-nshell, &   ! where the boundary occurs between root and soil (absorbing root node)
                                     z_node, &                ! elevation of all compartments [m]
                                     psi_node, &              ! water potential in compartment [Mpa]
                                     flc_node, &              ! frac loss conductivity [kg s-1 Mpa-1]
                                     dflcdpsi_node, &         ! change in FLC per change in water potential [kg s-1 Mpa-2]
                                     kmax_bound, &            ! max conductance at lower boundary of node [kg s-1 Mpa-1]
                                     kmax_upper, &
                                     kmax_lower, &
                                     hdiff_bound, &           ! out: difference in potential across nodes [Mpa]
                                     k_bound, &
                                     dhdiffdpsi0, &
                                     dhdiffdpsi1, &
                                     dkbounddpsi0, &
  ...
  subroutine boundary_hdiff_and_k(k_arootsoil, z_node, psi_node, flc_node, dflcdpsi_node, &
    ...
    do k = 1, (size(z_node)-1)
       hdiff_bound(k) = 1.e-6_r8*denh2o*grav_earth*(z_node(k) - z_node(k+1)) + &
