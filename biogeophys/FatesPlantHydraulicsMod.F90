...
contains 
    ...
    do s = 1,nsites
       ...
       case(campbell_type)
          do j=1,sites(s)%si_hydr%nlevsoi_hyd
             ...
             call wrf_cch%set_wrf_param([bc_in(s)%watsat_sisl(j), &  
                  (-1.0_r8)*bc_in(s)%sucsat_sisl(j)*denh2o*grav_earth*mpa_per_pa*m_per_mm , & 
                                      bc_in(s)%bsw_sisl(j)])
          end do
       case(tfs_type)
       ...
       case(campbell_type)
          do j=1,sites(s)%si_hydr%nlevsoi_hyd
             ...
             call wkf_cch%set_wkf_param([bc_in(s)%watsat_sisl(j), &
                  (-1.0_r8)*bc_in(s)%sucsat_sisl(j)*denh2o*grav_earth*mpa_per_pa*m_per_mm , & 
                  bc_in(s)%bsw_sisl(j)])
          end do
       case(tfs_type)
  ...
  subroutine UpdateTreeHydrNodes(ccohort_hydr,ft,plant_height,nlevsoi_hyd,bc_in)
    ...
    !write(fates_log(),*)'h: ',plant_height
    !write(fates_log(),*)'z upper: ', ccohort_hydr%z_upper_ag
    !write(fates_log(),*)'z nodes: ', ccohort_hydr%z_node_ag
    !write(fates_log(),*)'z lower: ', ccohort_hydr%z_lower_ag
    !write(fates_log(),*)'z troot: ', ccohort_hydr%z_node_troot
...
!          ccohort_hydr%v_aroot_layer(j)   = rootfr*v_aroot_tot
       ...
       !write(fates_log(),*)'vols: ', ccohort_hydr%v_ag,ccohort_hydr%v_troot
       !write(fates_log(),*)'v_aroot: ', ccohort_hydr%v_aroot_layer(:)
       !write(fates_log(),*)'l_aroot: ', ccohort_hydr%l_aroot_layer(:)
  ...
  subroutine UpdateSizeDepRhizVolLenCon(currentSite, bc_in)
    ...
    do j = 1,csite_hydr%nlevsoi_hyd
       ...
       hksat_s = bc_in%hksat_sisl(j) * m_per_mm * 1._r8/grav_earth * pa_per_mpa
    ...
    if(.false.) then
       ...
       do j = 1,csite_hydr%nlevsoi_hyd
          ...
          if( csite_hydr%l_aroot_layer(j) /= csite_hydr%l_aroot_layer_init(j) ) then
             ...
             s_shell_interp(j,k) = ( csite_hydr%wrf_soil(j)%p%th_from_psi(psi_shell_interp(j,k)) - bc_in%watres_sisl(j)) / & 
                  (bc_in%watres_sisl(j)+bc_in%watres_sisl(j))
       ...
       do j = 1,csite_hydr%nlevsoi_hyd
          ! proceed only if l_aroot_coh has changed
          if( csite_hydr%l_aroot_layer(j) /= csite_hydr%l_aroot_layer_init(j) ) then
             delta_s(j) = (( w_layer_init(j) - w_layer_interp(j) )/( v_rhiz(j) * denh2o ) - bc_in%watres_sisl(j)) / &
  ...
  subroutine hydraulics_bc ( nsites, sites,bc_in,bc_out,dtime )
    ...
    do s = 1, nsites
       ...
       delta_plant_storage  = site_hydr%h2oveg
       ...
       do j=1,site_hydr%nlevsoi_hyd
          ...
          watres_local = site_hydr%wrf_soil(j)%p%th_from_psi(bc_in(s)%smpmin_si*denh2o*grav_earth*m_per_mm*mpa_per_pa)

          do k=1,nshell
             if ((site_hydr%h2osoi_liqvol_shell(j,k)+dth_layershell_col(j,k)) > &
                  (bc_in(s)%watsat_sisl(j)-small_theta_num)) then

                ! We are destroying water in this case [kg/m2]
                supsub_error = supsub_error - site_hydr%v_shell(j,k)*denh2o*AREA_INV* & 
                     (site_hydr%h2osoi_liqvol_shell(j,k)+dth_layershell_col(j,k) - (bc_in(s)%watsat_sisl(j)-small_theta_num))

                ! Change dth_layer_shell to reflect the bounded soil moisture
                dth_layershell_col(j,k) = (bc_in(s)%watsat_sisl(j)-small_theta_num) - site_hydr%h2osoi_liqvol_shell(j,k)

                ! Flag that a superaturation problem was encountered here
                ...
                ! We are creating water [kg/m2]
                supsub_error = supsub_error + site_hydr%v_shell(j,k)*denh2o*AREA_INV* &
                     ((watres_local+small_theta_num) - (site_hydr%h2osoi_liqvol_shell(j,k)+dth_layershell_col(j,k)))
                
                ! Change dth_layer_shell to reflect the bounded soil moisture
                dth_layershell_col(j,k) = ( watres_local+small_theta_num ) - site_hydr%h2osoi_liqvol_shell(j,k)
                
                ! Flag that a super-saturation problem was encountered here
                site_hydr%supsub_flag(j)           = -k
       ...
       delta_plant_storage = site_hydr%h2oveg - delta_plant_storage
       ...
       if(abs(delta_soil_storage + root_flux - supsub_error) > 1.e-6_r8 ) then
       ...
       wb_error_site = site_hydr%h2oveg-(prev_h2oveg + totalrootuptake - transp_flux)

       site_hydr%h2oveg_hydro_err = site_hydr%h2oveg_hydro_err + wb_error_site + supsub_error
       ...
       !write(fates_log(),*) site_hydr%h2oveg_growturn_err
  ...
  subroutine UpdatePlantKmax(ccohort_hydr,ccohort,csite_hydr,bc_in)
    ...
    !write(fates_log(),*) 'ksl:',ccohort_hydr%kmax_stem_lower(:)
    !write(fates_log(),*) 'ktu: ',ccohort_hydr%kmax_troot_upper
    !write(fates_log(),*) 'ktl:',ccohort_hydr%kmax_troot_lower(:)
    !write(fates_log(),*) 'kau',ccohort_hydr%kmax_aroot_upper(:)
    !write(fates_log(),*) 'kri:',ccohort_hydr%kmax_aroot_radial_in(:)
    !write(fates_log(),*) 'kro:',ccohort_hydr%kmax_aroot_radial_out(:)
  ...
  subroutine ImTaylorSolve1D(cohort,cohort_hydr,site_hydr,bc_in,ilayer,dt_step,q_top, &
    ...
    ! Arguments (OUT)
    real(r8),intent(out) :: dth_node(n_hypool_tot) ! change in theta over the timestep
    ...
    integer,intent(out)  :: iter                   ! iteration count for sub-step loops
    ...
    do while( .not.solution_found ) 
       ...
       do istep = 1,nsteps
          ...
          do i = 1,n_hypool_plant
             ...
             psi_node(i) = wrf_plant(p_media_nodes(i),ft)%p%psi_from_th(th_node(i))
             ...
             ftc_node(i) = wkf_plant(p_media_nodes(i),ft)%p%ftc_from_psi(psi_node(i))

             ! deriv psi wrt theta
             dpsi_dtheta_node(i) = wrf_plant(p_media_nodes(i),ft)%p%dpsidth_from_th(th_node(i))

             ! deriv ftc wrt psi

             dftc_dpsi = wkf_plant(p_media_nodes(i),ft)%p%dftcdpsi_from_psi(psi_node(i))
          ...
          ! We dont allow any super-saturaiton in soil nodes
          ...
          ! Check if any psi values are > 0
          !if(any(psi_node(:) > nearzero)) then
          !   solution_found = .false.
          !   error_code = 4
          !   error_arr(:) = psi_node(:)
          !   exit
...
!!    end if
  ...
  subroutine Report1DError(cohort, site_hydr, bc_in, ilayer, z_node, v_node, & 
    ...
    do inode = 1,n_hypool_plant
       psi_node(inode) =  wrf_plant(p_media_nodes(inode),ft)%p%psi_from_th(th_node(inode)) 
       h_node(inode) =  mpa_per_pa*denh2o*grav_earth*z_node(inode) + psi_node(inode)
    end do
    do inode = n_hypool_plant+1,n_hypool_tot
       psi_node(inode) =  site_hydr%wrf_soil(ilayer)%p%psi_from_th(th_node(inode)) 
       h_node(inode) =  mpa_per_pa*denh2o*grav_earth*z_node(inode) + psi_node(inode)
    end do
  ...
  subroutine boundary_hdiff_and_k_alt(ccohort_hydr,psi_node,flc_node,dflcdpsi_node,hdiff_bound,k_bound,dhdpsi,dkdpsi)
    !
    ! !ARGUMENTS
    type(ed_cohort_hydr_type) :: ccohort_hydr
    real(r8) :: psi_node(num_nodes)
    real(r8) :: flc_node(num_nodes)
    real(r8) :: dflcdpsi_node(num_nodes)
    real(r8)          , intent(out) :: hdiff_bound(num_connections)         !total water potential difference across lower boundary          [MPa]
    real(r8)          , intent(out) :: dhdpsi(num_connections,2)         !
    real(r8)          , intent(out) :: k_bound(num_connections)         ! 
    real(r8)          , intent(out) :: dkdpsi(num_connections,2)         ! 
    real(r8) :: k_up,k_dn
    integer :: icnx,id_dn,id_up
    integer :: k_arootsoil
...
!
    ...
    associate( &
     ...
     k_arootsoil = n_hypool_tot-nshell
     do icnx = 1, num_connections
       id_dn = conn_dn(icnx)
       id_up = conn_up(icnx)
       hdiff_bound(icnx) = 1.e-6_r8*denh2o*grav*(z_node(id_dn) - z_node(id_up)) + (psi_node(id_dn) - psi_node(id_up))
       dhdpsi(icnx,1) = 1.d0 ! for id_dn
       dhdpsi(icnx,2) = -1.d0 ! for id_up
       do_kbound_upstream = .true.
       if(do_kbound_upstream) then
          if(icnx == (k_arootsoil)) then  ! absorbing root-1st rhizosphere shell boundary. Comprised of two distinct conductance terms each with distinct water potentials
             k_dn =  cond_dn(icnx) * flc_node(id_dn)
             k_up =  cond_up(icnx) * flc_node(id_up)
             k_bound(icnx)          =  1._r8/(1._r8/k_dn + 1._r8/k_up)
             dkdpsi(icnx,1)     =  ((k_bound(icnx)/k_dn)**2._r8) * cond_dn(icnx)*dflcdpsi_node(id_dn)
             dkdpsi(icnx,2)     =  ((k_bound(icnx)/k_up)**2._r8) * cond_up(icnx)*dflcdpsi_node(id_up)
          else
             ! examine direction of water flow; use the upstream node's k for
             ! the boundary k. (as suggested by Ethan Coon, LANL)
             if(hdiff_bound(icnx) < 0._r8) then
                k_bound(icnx)       = conductance(icnx) * flc_node(id_up)  !water moving towards atmosphere
                dkdpsi(icnx,1)  = 0._r8
                dkdpsi(icnx,2)  = conductance(icnx) * dflcdpsi_node(id_up)
             else
                k_bound(icnx)       = conductance(icnx) * flc_node(id_dn)    !water moving towards soil
                dkdpsi(icnx,1)  = conductance(icnx) * dflcdpsi_node(id_dn)
                dkdpsi(icnx,2)  = 0._r8
             end if
          end if
       else
...
! update location
       if(icnx == k_arootsoil) &
       k_arootsoil = k_arootsoil + nshell + 1
     enddo
    end associate

    return
!
  end subroutine boundary_hdiff_and_k_alt
...
!          dqtopdflcgs = 0.1411985_r8    !BOC... estimated by trial-and-error:this term multiplied by the maximum value of dflcgsdth gives 150.
                                        !NEEDED: an estimate for dqtopdflcgs
                                        !that accounts for variable potential
                                        !evapotranspiration (efpot).
                

       
!       psi_node(1:npool_bg) = psi_node_1l(1:npool_bg) 
!       th_node(1:npool_bg) = th_node_1l(1:npool_bg)
...
199       continue
          if( icnv == 1 ) then
             ...
             if( ntsr < 10 ) then
                   ...
                   dtcf = 0.2_r8
                   dtimex = dtime * dtcf
                   dtime = min(dtimex,tmx-tm)
                   dto = dtime
                   var = dtime
                   varx = dtx
                   write(*,'(4x,a,1pe11.4,1x,2a,1pe11.4,1x,a)')       &
                   'Time Step Reduced From ',varx,'s',' to ', &
                     var,'s'
                   do k = 1,num_nodes
...
!
                   ...
                   write(*,'(10x,a)') '---  Time Step Reduction Limit Exceeded---'
                   icnv = 4
                 endif
             endif
             do k=1,num_nodes
...
!     enddo
          ...
          do j = 1,site_hydr%nlevsoi_hyd
            ...
            enddo
          enddo
          dwat_veg_coh                          = dwat_veg_coh * denh2o
   ...
   ! =====================================================================================
  subroutine initHydrSolver(sites,bc_in)
       type(ed_site_type),intent(inout),target :: sites(:)
       type(bc_in_type),intent(in)             :: bc_in(:)
...
!
       ...
       do s = 1,nsites
          ...
          allocate(conn_dn(num_connections))
          allocate(conn_up(num_connections))
