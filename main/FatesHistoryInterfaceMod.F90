module FatesHistoryInterfaceMod
   ...
   integer :: ih_fire_fuel_moist_dead_si
   integer :: ih_fire_fuel_moist_live_si
   integer :: ih_fire_fuel_sav_si
   integer :: ih_fire_fuel_mef_dead_si
   integer :: ih_fire_fuel_mef_live_si
...
contains
   ...
   subroutine update_history_dyn_sitelevel(this,nc,nsites,sites)
      ...
      associate( hio_npatches_si         => this%hvars(ih_npatches_si)%r81d, &
         ...
         hio_fire_fuel_moist_dead_si => this%hvars(ih_fire_fuel_moist_dead_si)%r81d, &
         hio_fire_fuel_moist_live_si => this%hvars(ih_fire_fuel_moist_live_si)%r81d, &
         hio_fire_fuel_sav_si    => this%hvars(ih_fire_fuel_sav_si)%r81d, &
         hio_fire_fuel_mef_dead_si    => this%hvars(ih_fire_fuel_mef_dead_si)%r81d, &
         hio_fire_fuel_mef_live_si    => this%hvars(ih_fire_fuel_mef_live_si)%r81d, &
         hio_sum_fuel_si         => this%hvars(ih_sum_fuel_si)%r81d,  &
         hio_weighted_sum_fuel_si         => this%hvars(ih_weighted_sum_fuel_si)%r81d,  &
         ...
         siteloop: do s = 1,nsites
            ...
            patchloop: do while(associated(cpatch))
               ...
               hio_fire_fuel_bulkd_si(io_si)      = hio_fire_fuel_bulkd_si(io_si) + cpatch%fuel%bulk_density_weighted * cpatch%area * AREA_INV
               hio_fire_fuel_moist_dead_si(io_si)  = hio_fire_fuel_moist_dead_si(io_si) + cpatch%fuel%average_moisture_dead * cpatch%area * AREA_INV
               hio_fire_fuel_moist_live_si(io_si)  = hio_fire_fuel_moist_live_si(io_si) + cpatch%fuel%average_moisture_live * cpatch%area * AREA_INV
               hio_fire_fuel_sav_si(io_si)        = hio_fire_fuel_sav_si(io_si) + cpatch%fuel%SAV_weighted * cpatch%area * AREA_INV / m_per_cm
               hio_fire_fuel_mef_dead_si(io_si)   = hio_fire_fuel_mef_dead_si(io_si) + cpatch%fuel%MEF_dead * cpatch%area * AREA_INV
               hio_fire_fuel_mef_live_si(io_si)   = hio_fire_fuel_mef_live_si(io_si) + cpatch%fuel%MEF_live * cpatch%area * AREA_INV
               hio_sum_fuel_si(io_si)             = hio_sum_fuel_si(io_si) + cpatch%fuel%non_trunk_loading * cpatch%area * AREA_INV
               hio_weighted_sum_fuel_si(io_si)    = hio_weighted_sum_fuel_si(io_si) + (cpatch%fuel%weighted_loading_dead + &
                  cpatch%fuel%weighted_loading_live) * cpatch%area * AREA_INV
...
!         hio_fire_rate_of_spread_front_si_age => this%hvars(ih_fire_rate_of_spread_front_si_age)%r82d, &
   ...
   subroutine define_history_vars(this, initialize_variables)
      ...
      if_dyn0: if(hlm_hist_level_dynam>0) then
         ...
         call this%set_history_var(vname='FATES_FUEL_MEF_DEAD', units='m3 m-3',          &
            long='dead fuel moisture of extinction (volumetric)',                      &
            use_default='active', avgflag='A', vtype=site_r8, hlms='CLM:ALM',     &
            upfreq=group_dyna_simple, ivar=ivar, initialize=initialize_variables,                 &
            index=ih_fire_fuel_mef_dead_si)

         call this%set_history_var(vname='FATES_FUEL_MEF_LIVE', units='m3 m-3',          &
            long='live fuel moisture of extinction (volumetric)',                      &
            use_default='active', avgflag='A', vtype=site_r8, hlms='CLM:ALM',     &
            upfreq=group_dyna_simple, ivar=ivar, initialize=initialize_variables,                 &
            index=ih_fire_fuel_mef_live_si)
         ...
         call this%set_history_var(vname='FATES_FUEL_EFF_MOIST_DEAD', units='m3 m-3',    &
            long='spitfire dead fuel moisture (volumetric)', use_default='active',     &
            avgflag='A', vtype=site_r8, hlms='CLM:ALM', upfreq=group_dyna_simple, ivar=ivar,      &
            initialize=initialize_variables, index = ih_fire_fuel_moist_dead_si)

         call this%set_history_var(vname='FATES_FUEL_EFF_MOIST_LIVE', units='m3 m-3',    &
            long='spitfire live fuel moisture (volumetric)', use_default='active',     &
            avgflag='A', vtype=site_r8, hlms='CLM:ALM', upfreq=group_dyna_simple, ivar=ivar,      &
            initialize=initialize_variables, index = ih_fire_fuel_moist_live_si)
         ...
         call this%set_history_var(vname='FATES_FUEL_AMOUNT', units='kg m-2',       &
            long='total ground fuel not weighted by fuel SAV in kg C per m2 land area',   &
            use_default='active', avgflag='A', vtype=site_r8, hlms='CLM:ALM',     &
            upfreq=group_dyna_simple, ivar=ivar, initialize=initialize_variables,                 &
            index = ih_sum_fuel_si)

         call this%set_history_var(vname='FATES_FUEL_AMOUNT_WEIGHTED', units='kg m-2',       &
            long='total ground fuel weighted by fuel SAV and is related to FATES_ROS (omits 1000hr fuels) in kg C per m2 land area',   &
            use_default='active', avgflag='A', vtype=site_r8, hlms='CLM:ALM',     &
            upfreq=group_dyna_simple, ivar=ivar, initialize=initialize_variables,                 &
            index = ih_weighted_sum_fuel_si)
         ! Litter Variables
