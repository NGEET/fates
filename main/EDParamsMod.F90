module EDParamsMod
   ...
   real(r8),protected, public :: fnps                 ! Fraction of light absorbed by non-photosynthetic pigments
  ...
  ! BGC N and P pools, and there is also none removed.
   ...
   character(len=param_string_length),parameter,public :: ED_name_fnps = "fates_leaf_fnps"
...
contains
  ...
  subroutine FatesRegisterParams(fates_params)
    ...
    call fates_params%RegisterParameter(name=ED_name_fnps, dimension_shape=dimension_shape_scalar, &
         dimension_names=dim_names_scalar)
  ...
  subroutine FatesReceiveParams(fates_params)
    ...
    call fates_params%RetrieveParameter(name=ED_name_fnps, &
         data=fnps)
  ...
  subroutine FatesReportParams(is_master)
     ...
     if(debug_report .and. is_master) then
        ...
        write(fates_log(),fmt0) 'fnps =  ', fnps
