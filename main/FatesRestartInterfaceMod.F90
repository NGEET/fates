module FatesRestartInterfaceMod
  ...
  integer :: ir_gpp_tstep_co
  integer :: ir_npp_tstep_co
...
contains
 ...
 subroutine define_restart_vars(this, initialize_variables)
    ...
    if(hlm_use_sp .eq. itrue) then
         ...
         call this%set_restart_var(vname='fates_gpp_tstep', vtype=cohort_r8, &
             long_name='instantaneous fates gross primary production', &
             units='kgC/indiv/timestep', flushval = flushzero, &
             hlms='CLM:ALM', initialize=initialize_variables, ivar=ivar, index = ir_gpp_tstep_co )
         call this%set_restart_var(vname='fates_npp_tstep', vtype=cohort_r8, &
             long_name='instantaneous fates net primary production', &
             units='kgC/indiv/timestep', flushval = flushzero, &
             hlms='CLM:ALM', initialize=initialize_variables, ivar=ivar, index = ir_npp_tstep_co )
    end if
          ...
          do while(associated(cpatch))
             ...
             do while(associated(ccohort))
                ...
                if (hlm_use_sp .eq. itrue) then
                    ...
                    this%rvars(ir_gpp_tstep_co)%r81d(io_idx_co) = ccohort%gpp_tstep
                    this%rvars(ir_npp_tstep_co)%r81d(io_idx_co) = ccohort%npp_tstep
                end if
          ...
          do while(associated(cpatch))
             ...
             do while(associated(ccohort))
                ...
                if (hlm_use_sp .eq. itrue) then
                    ...
                    ccohort%npp_tstep = this%rvars(ir_npp_tstep_co)%r81d(io_idx_co)
                    ccohort%gpp_tstep = this%rvars(ir_gpp_tstep_co)%r81d(io_idx_co)
                end if
