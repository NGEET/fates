module FatesCohortMod
  ...
  contains
    ...
    subroutine init(this, prt)
      ...
      ! ARGUMENTS:
      type(fates_cohort_type), intent(inout), target  :: this
      class(prt_vartypes),     intent(inout), pointer :: prt ! allocated PARTEH object

      call this%nan_cohort()  ! make everything in the cohort not-a-number
      call this%zero_cohort() ! zero things that need to be zeroed
    ...
    function CanUpperUnder(this) result(can_position)
      ...
      ! ARGUMENTS:
      type(fates_cohort_type) :: this         ! current cohort of interest
    ...
    subroutine dump(this)
      ...
      ! ARGUMENTS:
      type(fates_cohort_type), intent(in), target :: this
      ...
      if (associated(this%co_hydr)) call dump_cohort_hydr(this)
