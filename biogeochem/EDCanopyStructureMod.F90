module EDCanopyStructureMod
  ...
  ! If we want to allow some degree of imperfection in canopy closer, we would
  ! add it here
  ...
  subroutine PromoteOrDemote(site,patch,target_layer,phase,target_area)
    ...
    associate(layer_co => patch%co_scr)
      ...
      ic_loop0: do ic = 1,n_layer
         ...
         ! If the demotion area is the same area as the
         !    cohort itself, demote the whole thing
         ! If the demotion area is less than the cohort area
         !    and not trivialy small (larger than precision
         !    check), then demote part of it
         ! If the demotion area is less than zero or larger than
