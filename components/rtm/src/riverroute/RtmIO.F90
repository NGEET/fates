...
!------------------------------------------------------------------------
  ...
  subroutine ncd_io_log_var1_nf(varname, data, flag, ncid, readvar, nt)
    ...
    elseif (flag == 'write') then
       ...
       status = pio_put_var(ncid, varid, start(1:ndims), count(1:ndims), idata)
       deallocate( idata )
  ...
  subroutine ncd_io_char_var1_nf(varname, data, flag, ncid, readvar, nt )
    ...
    elseif (flag == 'write') then
       ...
       if (present(nt))      then
          start(1) = 1
          start(2) = nt
          status = pio_put_var(ncid, varid, index=start, ival=data )
       else
...
!------------------------------------------------------------------------
  ...
  do n = 1,num_iodesc
     call pio_freedecomp(pio_subsystem, iodesc_list(n)%iodesc)
  end do
