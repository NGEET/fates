module CLMFatesInterfaceMod
   ...
   ! NOTE(bja, 2017-01) these methods can NOT be part of the interface
   ! type because they are called before the instance is initialized.
...
!         call this%fates2hlm%Init(bounds_proc)
 ...
 subroutine FatesReadParameters()
   ...
   if (use_ed) then
      if (masterproc) then
         write(iulog,*) 'clmfates_interfaceMod.F90::'//trim(subname)//' :: CLM reading ED/FATES '//' parameters '
      end if
 ...
 subroutine FatesReadPFTs()
   ...
   if (use_ed) then
      if (masterproc) then
         write(iulog,*) 'clmfates_interfaceMod.F90::'//trim(subname)//' :: CLM reading ED/FATES '//' PFTs '
      end if
